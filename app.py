...
#1.0b5 Added PTS, PPS, TPT%, TST%, ABR% to Shot Location Tables
...
def generate_team_report(conn, team_id, full_code):
    ...
    loc_table += f"""
                <tr style="font-weight:bold; background:#f2c74a; text-align:center;">
                    ...
                    <td>{loc_total_pts}</td>
                    <td>{loc_total_pps:.2f}</td>
                    <td>100%</td>
                    <td>100%</td>
                    <td>{loc_total_abr:.1f}%</td>
                </tr>
            </table>
        </div>
    </td>
...
def generate_game_report(conn, game_id: int) -> str:
    ...
    def team_stats(events, target_side, team_name):
        ...
        # Shot Location table
        parsed_locs = []
        t2pm = t2pa = t3pm = t3pa = total_ab = 0
        ...
        loc_table += f"""
                    <tr style='font-weight:bold; background:#75c875;'>
                        ...
                        <td style='text-align: center;'>{total_pts}</td><td style='text-align: center;'>{total_pps:.2f}</td>
                        <td style='text-align: center;'>100%</td><td style='text-align: center;'>100%</td>
                        <td style='text-align: center;'>{total_abr:.1f}%</td>
                    </tr>
                </table>
        </td><td valign='top'>
            <img src="https://raw.githubusercontent.com/CourtTag/courttag-assets/main/Shot_Location_Diagram_v1.svg" alt="Shot Location Diagram" style="width: 300px; height: 240px;">
